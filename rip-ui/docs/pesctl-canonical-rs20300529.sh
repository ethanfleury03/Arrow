...
# startServicing/startMovingPrintheads/startPriming/startDepriming are handled via real KPesClient below.
startPriming)
echo "__START_PRIMING_REAL__" ;;
startDepriming)
echo "__START_DEPRIMING_REAL__" ;;
replaceWipers|pausePrinting)
echo "__MAINT_STUB__" ;;
...
from memjet.psw.pdlib.thrift.pes_client import KPesClient
...
# Accept either numeric enum or string alias.
# Bridge mapping must be: raised=0, capped=1, print=2
if isinstance(position, (int, long)):
 pos_enum = int(position)
else:
 ...
 if pnorm not in map_str:
 raise RuntimeError('unsupported printhead position: {}'.format(position))
 pos_enum = map_str[pnorm]

client = KPesClient(host=host)
client.open_connection()
client.startMovingPrintheads(print_units, pos_enum)
print(json.dumps({
'ok': True,
'operation': op,
'host': host,
'commandPort': cmd_port,
'exitCode': 0,
'output': 'startMovingPrintheads sent (enum={})'.format(pos_enum)
}))
PY
)" || RC=$?
RC=${RC:-0}
...
if [[ "$CMDS" == "__START_PRIMING_REAL__" ]]; then
OUT="$(OP="$OP" ARGS_B64="$ARGS_B64" HOST="$HOST" CMD_PORT="$CMD_PORT" python2 - <<'PY'
import os, sys, json
...
called = None
for name in ('startPriming', 'prime', 'beginPriming'):
...
if not called:
 raise RuntimeError('no priming method available on KPesClient (tried startPriming, prime, beginPriming)')
...
print(json.dumps({
 ...
 'commandPort': cmd_port,
 'exitCode': 0,
 'output': 'priming sent via {}'.format(called)
}))
PY
)" || RC=$?
RC=${RC:-0}
...
if [[ "$CMDS" == "__START_DEPRIMING_REAL__" ]]; then
OUT="$(OP="$OP" ARGS_B64="$ARGS_B64" HOST="$HOST" CMD_PORT="$CMD_PORT" python2 - <<'PY'
import os, sys, json
...
called = None
for name in ('startDepriming', 'deprime', 'beginDepriming'):
 fn = getattr(client, name, None)
 if callable(fn):
  fn()
  called = name
...
if not called:
 raise RuntimeError('no depriming method available on KPesClient (tried startDepriming, deprime, beginDepriming)')
...
print(json.dumps({
 ...
 'commandPort': cmd_port,
 'exitCode': 0,
 'output': 'depriming sent via {}'.format(called)
}))
PY
)" || RC=$?
RC=${RC:-0}
